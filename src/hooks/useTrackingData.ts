...
export const useTrackingData = () => {
  ...
  const fetchUserTrackingData = async () => {
    ...
    try {
      ...
      // Fetch shipments from our database for the current user
      ...
      if (shipments && shipments.length > 0) {
        // Transform shipments data to TrackingInfo format
        const trackingInfos: TrackingInfo[] = shipments.map(shipment => {
          ...
          // Safe parsing of tracking events
        ...
        setTrackingData(trackingInfos);
        console.log(`Found ${trackingInfos.length} tracking records for user`);
      } else {
        setTrackingData([]);
