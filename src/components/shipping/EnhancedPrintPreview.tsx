...
const labelFormats = [{
  ...
  value: '8.5x11-top',
  label: '8.5x11" - 1 Shipping Label per Page - Top',
  description: 'One 4x6" label at the top of a letter-sized page'
}, {
  value: '8.5x11-bottom',
  label: '8.5x11" - 1 Shipping Label per Page - Bottom',
  description: 'One 4x6" label at the bottom of a letter-sized page'
}];
  ...
  return <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-5xl bg-white sm:rounded-lg h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader className="pb-3 border-b">
          ...
          <DialogClose asChild>
            <Button variant="ghost" size="icon" className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none">
              <X className="h-4 w-4" />
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
            ...
            <TabsContent value="download" className="flex-1 overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6">
                <div className="p-6 border-2 rounded-lg text-center cursor-pointer transition-all hover:shadow-lg border-blue-500 bg-blue-50 hover:bg-blue-100" onClick={() => handleDownload('pdf')}>
                ...
                <div className="p-6 border-2 rounded-lg text-center cursor-pointer transition-all hover:shadow-lg border-green-500 bg-green-50 hover:bg-green-100" onClick={() => handleDownload('png')}>
                ...
                <div className="p-6 border-2 rounded-lg text-center cursor-pointer transition-all hover:shadow-lg border-purple-500 bg-purple-50 hover:bg-purple-100" onClick={() => handleDownload('zpl')}>
              ...
              <div className="p-6 space-y-6 max-w-2xl mx-auto">
                <div>
                  ...
                  <div className="space-y-3">
                    {emailList.map((email, index) => <div key={index} className="flex gap-3">
                        <Input type="email" placeholder="Enter email address" value={email} onChange={e => updateEmailField(index, e.target.value)} className="flex-1 h-10" />
                        {emailList.length > 1 && <Button variant="outline" size="icon" onClick={() => removeEmailField(index)} className="text-red-600 hover:text-red-700 h-10 w-10">
                            <Trash2 className="h-4 w-4" />
                          </Button>}
                      </div>)}
                  </div>
                  <Button variant="outline" onClick={addEmailField} className="mt-3 h-10">
                ...
                <div>
                  ...
                  <Input type="text" placeholder="Enter email subject" value={emailSubject} onChange={e => setEmailSubject(e.target.value)} className="h-10" />
                </div>
                ...
                <Button onClick={handleSendEmail} className="w-full bg-blue-600 hover:bg-blue-700 text-white h-12 font-semibold text-lg">
