...
const EnhancedPrintPreview: React.FC<EnhancedPrintPreviewProps> = ({
  ...
  const generateLabelPDF = async (fileBytes: Uint8Array, layoutOption: string): Promise<Uint8Array> => {
    ...
    // Copy the first page from the original PDF to get embedded pages
    const embeddedPages = await outputPdf.copyPages(originalPdf, [0]);
    const embeddedPage = embeddedPages[0]; // Get the embedded page
  ...
  const handleDownload = async (format: 'pdf' | 'png' | 'zpl' | 'epl' = 'pdf') => {
    ...
    try {
      ...
      if (format === 'pdf' && originalPdfBytes) {
        // Use generated PDF with current format
        ...
        // For other formats or fallback, download directly from URL
        const response = await fetch(labelUrl);
        if (!response.ok) {
