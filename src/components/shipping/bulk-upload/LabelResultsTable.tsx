...
const LabelResultsTable: React.FC<LabelResultsTableProps> = ({
  ...
  const [previewModalOpen, setPreviewModalOpen] = useState<{ [key: string]: boolean }>({});
  ...
  const handleOpenPreview = (shipmentId: string) => {
    setPreviewModalOpen(prev => ({ ...prev, [shipmentId]: true }));
  };

  const handleClosePreview = (shipmentId: string) => {
    setPreviewModalOpen(prev => ({ ...prev, [shipmentId]: false }));
  };
        ...
        <table className="w-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {shipments.map((shipment, index) => {
              const shipmentKey = shipment.id || shipment.original_shipment_id || index;
              return (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex space-x-2">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => handleOpenPreview(shipmentKey)}
                        disabled={!(shipment.label_urls?.png || shipment.label_url)}
                      >
                      ...
                      <PrintPreview
                        isOpenProp={previewModalOpen[shipmentKey] || false}
                        onOpenChangeProp={(isOpen) => {
                          if (!isOpen) handleClosePreview(shipmentKey);
                        }}
