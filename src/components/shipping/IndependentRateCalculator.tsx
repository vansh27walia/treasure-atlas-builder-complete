...
const IndependentRateCalculator: React.FC = () => {
  ...
  // Removed the countries array and related state, hardcoding 'US'
  const originCountry = 'US';
  const destCountry = 'US';
  const isInternational = false; // Always false now
  ...
  const fetchRates = async () => {
    ...
    try {
      ...
      // Simplified API calls, hardcoding US
      const originQuery = `${originZip}, United States`;
      ...
      const destQuery = `${destZip}, United States`;
      ...
      if (data.rates && Array.isArray(data.rates)) {
        ...
        toast.success(`Found ${processedRates.length} domestic shipping rates`);
      } else {
        setRates([]);
  ...
  const handleShipThis = (rate: RateResult) => {
    const calculatorData = {
      originZip,
      destZip,
      originCountry, // Will always be 'US'
      destCountry, // Will always be 'US'
      packageType,
      dimensions,
      selectedRate: rate,
      isInternational, // Will always be false
      insuranceEnabled,
      insuranceAmount,
    ...
    <div className="container mx-auto px-6 py-8 max-w-6xl">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent mb-2">
          Smart Domestic Rate Calculator
        </h1>
      ...
      <Card className="mb-8 shadow-xl border-0 bg-white/90 backdrop-blur-sm">
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            <Label htmlFor="insurance-toggle" className="text-lg">
              Include default insurance coverage
            </Label>
            <div className="flex items-center gap-2">
              <Switch id="insurance-toggle" checked={insuranceEnabled} onCheckedChange={setInsuranceEnabled} />
              <div className="text-sm text-gray-500">($100 coverage for $2)</div>
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                <Input value={originZip} onChange={e => setOriginZip(e.target.value)} placeholder="Enter ZIP code (e.g., 90210)" className="h-12 text-base border-2 border-gray-200 focus:border-blue-500" />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                <Input value={destZip} onChange={e => setDestZip(e.target.value)} placeholder="Enter ZIP code (e.g., 10001)" className="h-12 text-base border-2 border-gray-200 focus:border-blue-500" />
              </div>
            </div>
          </div>
        </CardContent>
          ...
          <CardHeader>
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center">
              <CardTitle className="text-2xl text-blue-800 flex items-center gap-2">
                ...
                Available Domestic Rates ({rates.length})
              </CardTitle>
