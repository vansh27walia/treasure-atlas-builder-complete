...
const labelFormats = [{
  value: '4x6',
  label: '4x6" Thermal Printer',
  description: 'Standard thermal label size for direct printing'
}, {
  value: '8.5x11-2up',
  label: '8.5x11" - 2 Labels (2-up)',
  description: 'Two labels per page - top and bottom'
}, {
  value: '8.5x11-top',
  label: '8.5x11" - Single (Top)',
  description: 'One label at top of letter page'
}, {
  value: '8.5x11-bottom',
  label: '8.5x11" - Single (Bottom)',
  description: 'One label at bottom of letter page'
}];
...
const PrintPreview: React.FC<PrintPreviewProps> = ({
  ...
  // Download the currently displayed/formatted PDF
  const handleDownloadCurrentFormat = async () => {
    try {
      toast.loading(`Preparing ${selectedFormat} PDF download...`);
      let blob: Blob;
      const filename = `shipping_label_${selectedFormat}_${trackingCode || shipmentId || Date.now()}.pdf`;

      // If we have a formatted PDF in currentPreviewUrl (blob URL), use that
      if (currentPreviewUrl && currentPreviewUrl.startsWith('blob:')) {
        const response = await fetch(currentPreviewUrl);
        blob = await response.blob();
      } else if (originalPdfBytes) {
        // Generate PDF with current format selection
      ...
      toast.dismiss();
      toast.success(`${selectedFormat} PDF downloaded successfully`);
    } catch (error) {
  ...
  const dialogTitleText = isBatchPreview ? `Batch Operations (ID: ${batchResult?.batchId || 'N/A'})` : `Shipping Label Preview ${trackingCode ? `(${trackingCode})` : ''}`;
  return <Dialog open={isOpen} onOpenChange={setIsOpen}>
      {triggerButton ? <DialogTrigger asChild>
          {triggerButton}
        </DialogTrigger> : <div className="flex gap-2">
          <Button variant="outline" size="sm" className="border-blue-200 hover:bg-blue-50 text-blue-700" onClick={() => handleDownload('pdf')} disabled={!originalPdfBytes && !labelUrls?.pdf}>
      ...
      <DialogContent className="max-w-5xl w-[95vw] bg-white sm:rounded-lg max-h-[95vh] h-[95vh] flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0 pb-2">
          <DialogTitle className="flex items-center justify-between pr-6 text-base">
            <span>{dialogTitleText}</span>
          </DialogTitle>
          <Button variant="ghost" size="icon" onClick={() => setIsOpen(false)} disabled={isRegeneratingLabel} className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none">
            <X className="h-4 w-4" />
            <span className="sr-only">Close</span>
          </Button>
        </DialogHeader>

        <div className="flex-1 flex flex-col pt-2 overflow-hidden min-h-0">
          {/* Tabs for Preview/Download/Email */}
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden">
            {!isBatchPreview && <TabsList className="grid w-full grid-cols-3 mb-4 h-10 flex-shrink-0">
            ...
            <TabsContent value="preview" className="flex-1 flex flex-col overflow-hidden m-0">
              {/* Full-width Format Selection at Top */}
              <div className="flex-shrink-0 w-full bg-gray-50 border rounded-lg p-4 mb-4">
                <div className="flex items-center gap-4">
                  <Label className="text-sm font-semibold whitespace-nowrap text-gray-700">Label Format:</Label>
                  <Select value={selectedFormat} onValueChange={handleFormatChange} disabled={isRegeneratingLabel}>
                    <SelectTrigger className="flex-1 h-11 bg-white border-2 border-gray-300 hover:border-blue-400 focus:border-blue-500 text-base font-medium">
                      <SelectValue placeholder="Select Format" />
                    </SelectTrigger>
                    <SelectContent className="bg-white border border-gray-300 shadow-lg z-[60]">
                      {labelFormats.map(format => <SelectItem key={format.value} value={format.value} className="cursor-pointer hover:bg-gray-50 py-3">
              ...
              <div className="flex-1 flex-col bg-gray-100 border-gray-300 relative overflow-hidden min-h-0 mx-0 px-0 flex items-center justify-center rounded-2xl opacity-100 border-none border">
                {isRegeneratingLabel && <div className="absolute inset-0 bg-white/80 flex items-center justify-center z-10">
                ...
                <div className="w-full h-full flex items-center justify-center p-4">
                  {isRegeneratingLabel ? <div className="flex items-center justify-center h-full">
                      <div className="flex flex-col items-center">
                        <Loader2 className="h-8 w-8 animate-spin text-purple-600 mb-2" />
                        <p className="text-sm text-purple-800">Regenerating...</p>
                      </div>
                    </div> : previewType === 'pdf' && currentPreviewUrl ? <iframe ref={iframeRef} src={currentPreviewUrl} className="w-full h-full border border-gray-300 rounded-lg bg-white shadow-lg" style={{
                  minHeight: '400px'
                }} title="Label Preview" onLoad={() => console.log('PDF iframe loaded successfully')} onError={e => {
                  console.error('PDF iframe error:', e);
                  toast.error('Failed to load PDF preview');
                }} /> : previewType === 'image' && currentPreviewUrl ? <img src={currentPreviewUrl} alt="Shipping Label" className="max-w-full max-h-full h-auto border border-gray-300 rounded-lg shadow-lg object-contain" onLoad={() => console.log('Image loaded successfully')} onError={e => {
                  console.error('Image load error:', e);
                  toast.error('Failed to load image preview');
                }} /> : <div className="flex items-center justify-center h-full text-gray-500">
                      {isBatchPreview && !batchResult?.consolidatedLabelUrls?.pdf ? <div className="text-center">
                          <Files className="h-16 w-16 mx-auto mb-4 text-gray-300" />
                          <p className="text-lg">Batch PDF needed for preview.</p>
                        </div> : <div className="text-center">
                          <Package className="h-16 w-16 mb-4 text-gray-400 mx-auto" />
              ...
              <div className="flex-shrink-0 grid grid-cols-2 gap-4 pt-4 mt-4 border-t">
                <Button onClick={handlePrint} disabled={isRegeneratingLabel || !currentPreviewUrl} className="w-full bg-blue-600 hover:bg-blue-700 text-white h-12 text-base font-semibold rounded-lg shadow-md">
                  <Printer className="h-5 w-5 mr-2" />
                  Print Label
                </Button>
                <Button onClick={handleDownloadCurrentFormat} disabled={isRegeneratingLabel || !currentPreviewUrl} className="w-full bg-green-600 hover:bg-green-700 text-white h-12 text-base font-semibold rounded-lg shadow-md">
                  <Download className="h-5 w-5 mr-2" />
                  Download {selectedFormat} PDF
                </Button>
              </div>
            </TabsContent>
              ...
              <div className="p-4 space-y-4 max-w-xl mx-auto">
                <div>
                  ...
                  <div className="space-y-2">
                    {emailList.map((email, index) => <div key={index} className="flex gap-2">
                        <Input type="email" placeholder="Enter email address" value={email} onChange={e => updateEmailField(index, e.target.value)} className="flex-1 h-9" />
                        {emailList.length > 1 && <Button variant="outline" size="icon" onClick={() => removeEmailField(index)} className="text-red-600 hover:text-red-700 h-9 w-9">
                ...
                <div>
                  ...
                  <Input type="text" placeholder="Enter email subject" value={emailSubject} onChange={e => setEmailSubject(e.target.value)} className="h-9" />
                </div>
                ...
                <Button onClick={handleSendEmail} className="w-full bg-blue-600 hover:bg-blue-700 text-white h-10 font-semibold">
                  <Mail className="h-4 w-4 mr-2" />
                  Send Email
                </Button>
              </div>
        ...
        <DialogFooter className="flex-shrink-0 sm:justify-between pt-3 border-t gap-2">
          <div className="flex gap-2">
            {!isBatchPreview && shipmentId && trackingCode && shipmentDetails && <CancelLabelDialog shipmentId={shipmentId} trackingCode={trackingCode} carrier={shipmentDetails.carrier} service={shipmentDetails.service} fromAddress={shipmentDetails.fromAddress} toAddress={shipmentDetails.toAddress} />}
          </div>
          <DialogClose asChild>
