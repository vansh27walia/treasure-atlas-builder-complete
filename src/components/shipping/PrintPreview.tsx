...
    const originalPdf = await PDFDocument.load(fileBytes);
    ...
    // Guard against multi-page PDFs which are not supported for this client-side reformatting
    if (originalPdf.getPages().length > 1) {
        throw new Error("Cannot reformat multi-page label PDF on the client-side.");
    }
    const outputPdf = await PDFDocument.create();

    // The index of the first page is 0
    const originalPage = await outputPdf.embedPage(originalPdf.getPages()[0]);
    
    // Get the original dimensions of the label (e.g., 4x6 inches in points)
...
    const margin = 30; // 0.41 inches, used for border spacing
    
    if (layoutOption === '4x6') {
        // For 4x6, use the original label dimensions directly
      const page = outputPdf.addPage([originalLabelWidth, originalLabelHeight]);
      page.drawPage(originalPage);
    } else {
        // For 8.5x11 layouts (2up, top, bottom), we need rotation
      const page = outputPdf.addPage([letterWidth, letterHeight]);
      
        // When rotated 90 degrees, the original height (432) becomes the new width on the page.
      const rotatedLabelWidth = originalLabelHeight; // 432
      const rotatedLabelHeight = originalLabelWidth; // 288
      
        // Calculate centered X position. This is the X coordinate for the drawPage origin.
        // With a 90-degree rotation around (x,y), the resulting content's left edge is at x.
      const xPlacement = (letterWidth - rotatedLabelWidth) / 2; // (612 - 432) / 2 = 90
      
        // Calculate Y position for the origin.
        // Rotation 90 degrees: The resulting content's height runs from (y - rotatedLabelHeight) to y.
        // Therefore, y is the TOP edge of the drawn label in the new coordinate system.

      if (layoutOption === '8.5x11-2up') {
        // Top label: The top edge (y) should be letterHeight - margin
        const topYOrigin = letterHeight - margin; // 792 - 30 = 762
        // Bottom label: The bottom edge (y - rotatedLabelHeight) should be margin.
        // So, y = margin + rotatedLabelHeight
        const bottomYOrigin = margin + rotatedLabelHeight; // 30 + 288 = 318

        // Draw the top label
        page.drawPage(originalPage, {
          x: xPlacement, // Correctly centered
          y: topYOrigin, // Correctly positioned near the top margin
          rotate: degrees(90),
        });
...
        page.drawPage(originalPage, {
          x: xPlacement, // Correctly centered
          y: bottomYOrigin, // Correctly positioned near the bottom margin
          rotate: degrees(90),
        });
      } else if (layoutOption === '8.5x11-top') {
        // Top label only: The top edge (y) should be letterHeight - margin
        const topYOrigin = letterHeight - margin; // 762
...
      } else if (layoutOption === '8.5x11-bottom') {
        // Bottom label only: The top edge (y) should be margin + rotatedLabelHeight
        const bottomYOrigin = margin + rotatedLabelHeight; // 318
