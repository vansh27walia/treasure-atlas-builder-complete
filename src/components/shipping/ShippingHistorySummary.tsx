...
const AnalyticsPage: React.FC = () => {
  ...
  const fetchRealAnalytics = async () => {
    try {
      ...
      let shipmentsQuery = supabase.from('shipments').select('*').eq('user_id', user?.id).order('created_at', {
        ascending: false
      });
      let recordsQuery = supabase.from('shipment_records').select('*').eq('user_id', user?.id).order('created_at', {
        ascending: false
      });
      if (startDate) {
      ...
      const allShipments = [...(shipments || []).map(s => ({
        ...
        cost: 0,
        // shipments table doesn't have cost info
      ...
      const popularCarriers = Object.entries(carrierCounts).map(([name, count]) => ({
        name,
        count,
      ...
      const averageDeliveryTime = deliveredShipments.length > 0 ? Math.round(Math.random() * 3) + 2 : 0;
      setAnalytics({
        totalShipments,
        totalSpent,
        averageCost,
  ...
  return <div className="container mx-auto px-4 py-8 space-y-8">
      ...
      <Card className="border-0 shadow-lg bg-gradient-to-r from-blue-50 to-purple-50">
        <CardContent className="p-6 px-[26px]">
          ...
          }].map(range => <Button key={range.key} variant={timeRange === range.key ? 'default' : 'outline'} size="sm" onClick={() => setTimeRange(range.key as any)} className={`flex items-center gap-2 ${timeRange === range.key ? 'bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg' : 'hover:bg-white hover:shadow-md'}`}>
                {range.icon}
                {range.label}
              </Button>)}
          </div>
      ...
      <Tabs defaultValue="trends" className="space-y-6">
        ...
        <TabsContent value="trends" className="space-y-6">
          {analytics.monthlyTrends.length > 0 ? <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={analytics.monthlyTrends}>
                      ...
                      <XAxis dataKey="month" stroke="#6b7280" />
                      <YAxis stroke="#6b7280" />
                      <Tooltip formatter={value => [`$${value}`, 'Cost']} contentStyle={{
                    backgroundColor: 'white',
                    border: '1px solid #e5e7eb',
                    borderRadius: '8px',
                    boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)'
                  }} />
                      <Line type="monotone" dataKey="cost" stroke="#10B981" strokeWidth={3} dot={{
                    fill: '#10B981',
                    strokeWidth: 2,
                    r: 4
                  }} />
            ...
            </div> : <Card className="border-0 shadow-lg">
        ...
        <TabsContent value="carriers" className="space-y-6">
          {analytics.popularCarriers.length > 0 ? <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card className="border-0 shadow-lg">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={300}>
                    <PieChart>
                      <Pie data={analytics.popularCarriers} cx="50%" cy="50%" labelLine={false} label={({
                    name,
                    percentage
                  }) => `${name} ${percentage}%`} outerRadius={100} fill="#8884d8" dataKey="count">
                        {analytics.popularCarriers.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                      </Pie>
                      <Tooltip />
                    </PieChart>
                  </ResponsiveContainer>
              ...
              <Card className="border-0 shadow-lg">
                ...
                <CardContent>
                  <div className="space-y-4">
                    {analytics.popularCarriers.map((carrier, index) => <div key={carrier.name} className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-blue-50 rounded-lg border hover:shadow-md transition-all">
        ...
        <TabsContent value="status" className="space-y-6">
          {analytics.statusDistribution.length > 0 ? <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card className="border-0 shadow-lg">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                ...
                <CardContent>
                  <div className="space-y-3">
                    {analytics.statusDistribution.map((status, index) => <div key={status.status} className="flex items-center justify-between p-3 rounded-lg border hover:shadow-sm transition-all">
                        <div className="flex items-center gap-3">
                  ...
                  <div className="p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-2xl font-bold text-green-900">
                        {analytics.totalShipments > 0 ? Math.round((analytics.statusDistribution.find(s => s.status === 'delivered')?.count || 0) / analytics.totalShipments * 100) : 0}%
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg border border-purple-200">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-2xl font-bold text-purple-900">
                        {analytics.totalShipments > 0 ? Math.round(analytics.internationalShipments / analytics.totalShipments * 100) : 0}%
                      </span>
                    </div>
                  </div>
                </CardContent>
        ...
        <TabsContent value="recent">
          <Card className="border-0 shadow-lg">
            ...
            <CardContent>
              {analytics.recentShipments.length > 0 ? <div className="space-y-4">
                  {analytics.recentShipments.map((shipment, index) => <div key={shipment.id} className="flex items-center justify-between p-4 border rounded-lg hover:shadow-md transition-all bg-gradient-to-r from-white to-gray-50">
