...
const Index: React.FC = () => {
  ...
  const fetchRealDashboardData = async () => {
    try {
      ...
      const [shipmentsResponse, recordsResponse] = await Promise.all([
        supabase
          .from('shipments')
          .select('*')
          .eq('user_id', user?.id)
      ...
      const inTransitShipments = allShipments.filter(s => 
        ['in_transit', 'created', 'pending', 'processing'].includes(s.status || '')
      ).length;
      ...
      thisMonth.setDate(1);
      const thisMonthShipments = allShipments.filter(s => 
        new Date(s.created_at) >= thisMonth
      ).length;
      const thisMonthSpent = records.filter(r => 
  ...
  const handleDownloadAnalytics = async () => {
    try {
      ...
      const [shipmentsRes, recordsRes] = await Promise.all([
        ...
        supabase.from('shipment_records').select('*').eq('user_id', user?.id)
      ]);
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {quickActionItems.map((item, index) => (
              <Card key={index} className={`hover:scale-105 transition-all duration-300 cursor-pointer border-0 shadow-xl bg-gradient-to-br ${item.gradient} hover:shadow-2xl`} onClick={item.action}>
          ...
          <Card className="col-span-1 lg:col-span-2 border-0 shadow-xl bg-white">
            ...
            <CardContent className="p-6">
              {!user ? (
                ...
                <div className="space-y-4">
                  {dashboardData.recentShipments.slice(0, 4).map((shipment: any, index) => (
        ...
        <div className="text-center bg-gradient-to-r from-blue-600 via-purple-600 to-teal-600 rounded-xl p-12 text-white shadow-2xl">
          ...
          <div className="flex justify-center gap-6 flex-wrap">
            ...
            <Button 
              ...
              variant="outline"
              className="border-2 border-white text-white hover:bg-white hover:text-blue-600 shadow-xl px-8 py-4 text-lg"
            >
