  ...
  useEffect(() => {
    const fetchRealData = async () => {
      try {
        ...
        const {
          data: shipments
        } = await supabase.from('shipments').select('*').order('created_at', {
          ascending: false
        }).limit(10);
      ...
      <div className="p-8 space-y-8">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {quickActionItems.map((item, index) => <Card key={index} className={`hover:scale-105 transition-all duration-200 cursor-pointer border-0 shadow-lg bg-gradient-to-br ${item.gradient}`} onClick={item.action}>
          ...
          <Card className="border-0 shadow-lg bg-gradient-to-br from-green-50 to-green-100">
            ...
            <CardContent>
              ...
              <p className="text-xs text-green-600 mt-1">
                {dashboardData.totalShipments > 0 ? `${Math.round(dashboardData.deliveredShipments / dashboardData.totalShipments * 100)}% success rate` : '93% success rate'}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="col-span-1 lg:col-span-2 border-0 shadow-lg bg-white">
            ...
            <CardContent className="p-6">
              {loading ? <div className="space-y-3">
                  ...
                  {dashboardData.recentShipments.slice(0, 3).map((shipment: any, index) => <div key={index} className="flex justify-between items-center p-4 border border-gray-200 rounded-lg bg-gradient-to-r from-gray-50 to-blue-50 hover:shadow-md transition-all">
                      <div>
                ...
                code: 'TRK1003',
                status: 'DELIVERED'
              }].map((shipment, index) => <div key={index} className="flex justify-between items-center p-4 border border-gray-200 rounded-lg bg-gradient-to-r from-gray-50 to-blue-50 hover:shadow-md transition-all">
                      <div>
                        <p className="font-medium text-gray-900">{shipment.carrier} - {shipment.code}</p>
                        <p className={`text-sm font-medium ${shipment.status === 'DELIVERED' ? 'text-green-600' : shipment.status === 'IN TRANSIT' ? 'text-blue-600' : 'text-gray-600'}`}>
                          {shipment.status}
                        </p>
                      </div>
                      <div className="text-right">
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  {dashboardData.topCarriers.map((carrier, index) => <div key={index} className="flex justify-between items-center p-2 bg-gray-50 rounded-lg">
        ...
        <div className="text-center bg-gradient-to-r from-blue-600 via-purple-600 to-teal-600 rounded-lg p-8 text-white shadow-xl">
          ...
          <div className="flex justify-center gap-4 flex-wrap">
            <Button size="lg" onClick={() => navigate('/create-label')} className="bg-white text-blue-600 hover:bg-blue-50 shadow-lg">
              <Package className="mr-2 h-5 w-5" />
              Create Single Label
            </Button>
            <Button size="lg" onClick={() => navigate('/bulk-upload')} variant="outline" className="border-white text-white hover:bg-white hover:text-blue-600 shadow-lg">
