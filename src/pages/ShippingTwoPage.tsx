...
import EnhancedShippingTwoForm from '@/components/shipping/EnhancedShippingTwoForm';
import ShippingTwoRates from '@/components/shipping/ShippingTwoRates';
import RateCalculator from '@/components/shipping/RateCalculator';
import { ShippingStep } from '@/types/shipping';

const ShippingTwoPage: React.FC = () => {
  ...
  useEffect(() => {
    const handleStepChange = (event: CustomEvent<{step: ShippingStep}>) => {
      ...
      <div className="max-w-7xl mx-auto">
        <Card className="border border-gray-200 shadow-md bg-white rounded-lg overflow-hidden">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid w-full grid-cols-4 mb-4 bg-indigo-50 p-1 rounded-lg">
              <TabsTrigger 
                value="domestic" 
            ...
            <TabsContent value="domestic" className="p-4">
              <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg mb-6 border border-indigo-100 shadow-sm p-4">
                <h2 className="text-lg font-semibold text-indigo-800 flex items-center mb-2">
                  <Truck className="h-5 w-5 mr-2 text-indigo-600" />
                  Enhanced Domestic Shipping
                </h2>
                <p className="text-indigo-700 text-sm">Ship packages within the country with our improved workflow and carrier options.</p>
              </div>
              
              <EnhancedShippingTwoForm />
            </TabsContent>
            
            <TabsContent value="international" className="p-4">
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg mb-6 border border-blue-100 shadow-sm p-4">
                <h2 className="text-lg font-semibold text-blue-800 flex items-center mb-2">
                  <Globe className="h-5 w-5 mr-2 text-blue-600" />
                  International Shipping
                </h2>
                <p className="text-blue-700 text-sm">Send packages worldwide with customs forms automatically generated.</p>
              </div>
            ...
            <TabsContent value="calculator" className="p-4">
              <div className="bg-gradient-to-r from-green-50 to-teal-50 rounded-lg mb-6 border border-green-100 shadow-sm p-4">
            ...
            <TabsContent value="bulk" className="p-4">
              <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg mb-6 border border-amber-100 shadow-sm p-4">
        ...
        {activeTab === 'domestic' && <ShippingTwoRates />}
        {activeTab === 'calculator' && <ShippingTwoRates />}
      </div>
    </div>
  );
};

export default ShippingTwoPage;
