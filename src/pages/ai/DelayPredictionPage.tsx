...
const DelayPredictionPage: React.FC = () => {
  const { user } = useAuth();
  const { isLoading, predictions, predictDelays } = useAILogistics();
  ...
  const getDelayTypeIcon = (type: string) => {
    switch (type) {
      case 'customs': return '🛃';
      case 'carrier_backlog': return '📦';
      case 'weather': return '🌧️';
      case 'missed_scan': return '📡';
      case 'handoff_failure': return '🔄';
  ...
  const getDelayTypeLabel = (type: string) => {
    switch (type) {
      case 'customs': return 'Customs Hold';
      case 'carrier_backlog': return 'Carrier Backlog';
      case 'weather': return 'Weather Disruption';
      case 'missed_scan': return 'Missed Scan';
      case 'handoff_failure': return 'Handoff Failure';
      case 'lost_package': return 'Lost Package Risk';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-orange-900 to-slate-900 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Summary Card */}
        {summary && (
          <Card className="bg-gradient-to-r from-orange-800/50 to-red-800/50 border-orange-500/30 backdrop-blur-sm">
            <CardContent className="pt-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-white font-semibold mb-2">AI Risk Summary</h3>
                  <p className="text-orange-200">{summary}</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="bg-slate-800/80 border-slate-700 backdrop-blur-sm">
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="space-y-4">
                {predictions.sort((a, b) => b.delay_probability - a.delay_probability).map((prediction) => (
                  <div 
                    key={prediction.id} 
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                          <div>
                            ...
                            <Progress 
                              value={prediction.delay_probability * 100} 
                              className="h-2"
                            />
                          </div>
                          <div>
                        ...
                        {/* Predicted Delay */}
                        {prediction.predicted_delay_hours > 0 && (
                        ...
                        {prediction.reasoning && (
                          <p className="text-slate-400 text-sm mb-3">{prediction.reasoning}</p>
                        )}
                        ...
                        {prediction.suggested_actions && prediction.suggested_actions.length > 0 && (
                          <div className="space-y-2">
                            ...
                            {prediction.suggested_actions.map((action: string, i: number) => (
                      ...
                      <div className={`px-4 py-2 rounded-xl ${getRiskColor(prediction.delay_probability)}`}>
                        ...
                        <p className="text-2xl font-bold">
                          {prediction.delay_probability >= 0.8 ? 'Critical' :
                           prediction.delay_probability >= 0.6 ? 'High' :
                           prediction.delay_probability >= 0.4 ? 'Medium' : 'Low'}
                        </p>
                      </div>
                    </div>
                  </div>
