...
import { Package, Search, Brain, AlertTriangle, CheckCircle2, Clock, TrendingUp, RefreshCw, ChevronRight, MapPin, Truck } from 'lucide-react';
...
const ShipmentIntelligencePage: React.FC = () => {
  ...
  const filteredShipments = shipments.filter(s => s.tracking_code?.toLowerCase().includes(searchQuery.toLowerCase()) || s.carrier?.toLowerCase().includes(searchQuery.toLowerCase()) || s.to_address_json?.city?.toLowerCase().includes(searchQuery.toLowerCase()));
  const existingInsight = selectedShipment ? insights.find(i => i.shipment_id === selectedShipment.shipment_id) : null;
  return <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-6">
            ...
            <Card className="bg-slate-800/80 border-slate-700 backdrop-blur-sm">
              <CardHeader className="pb-3">
                <div className="relative">
                  ...
                  <Input placeholder="Search shipments..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9 bg-slate-700 border-slate-600 text-white placeholder:text-slate-400" />
                </div>
              </CardHeader>
              <CardContent className="space-y-2 max-h-[600px] overflow-y-auto">
                {loadingShipments ? Array(5).fill(0).map((_, i) => <Skeleton key={i} className="h-20 w-full" />) : filteredShipments.length > 0 ? filteredShipments.map(shipment => <div key={shipment.id} onClick={() => handleAnalyze(shipment)} className={`p-4 rounded-lg border cursor-pointer transition-all ${selectedShipment?.id === shipment.id ? 'bg-blue-600/30 border-blue-500' : 'bg-slate-700/50 border-slate-600 hover:border-slate-500'}`}>
          ...
          <div className="lg:col-span-2 space-y-4">
            {selectedShipment ? <>
                ...
                <Card className="bg-slate-800/80 border-slate-700 backdrop-blur-sm">
                  <CardHeader>
                    <CardTitle className="text-white flex items-center justify-between">
                      ...
                      <Button size="sm" onClick={() => handleAnalyze(selectedShipment)} disabled={isLoading} className="bg-blue-600 hover:bg-blue-700">
                ...
                <Card className="bg-gradient-to-r from-blue-800/50 to-cyan-800/50 border-blue-500/30 backdrop-blur-sm">
                  ...
                  <CardContent>
                    {analysisError || rateLimited ? <div className="text-center py-8 text-slate-300">
                        ...
                        <div className="mt-4">
                          <Button onClick={() => handleAnalyze(selectedShipment)} disabled={isLoading} variant="outline">
                            <RefreshCw className={`w-4 h-4 mr-2 ${isLoading ? 'animate-spin' : ''}`} />
                            Retry
                          </Button>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          <div className="bg-slate-700/50 rounded-lg p-3">
                            ...
                            <div className="flex items-center gap-2">
                              <Progress value={(analysis?.confidenceScore || existingInsight?.confidence_score || 0) * 100} className="h-2 flex-1" />
                          ...
                          <div className="bg-slate-700/50 rounded-lg p-3">
                            ...
                            <p className="text-white text-sm">
                              {analysis?.predictedDeliveryDate ? new Date(analysis.predictedDeliveryDate).toLocaleDateString() : existingInsight?.predicted_delivery_date ? new Date(existingInsight.predicted_delivery_date).toLocaleDateString() : 'N/A'}
                            </p>
                          </div>
                        </div>
                        ...
                        {/* Recommendations */}
                        {(analysis?.recommendations || existingInsight?.recommendations)?.length > 0 && <div>
                            <h4 className="text-cyan-300 text-sm font-medium mb-3">Recommendations</h4>
                            <div className="space-y-2">
                              {(analysis?.recommendations || existingInsight?.recommendations || []).map((rec: string, i: number) => <div key={i} className="flex items-start gap-2 bg-slate-700/50 rounded-lg p-3">
